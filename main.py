...
def remove_property_if_value(obj, property_name, value):
    if isinstance(obj, dict):
        return {k: remove_property_if_value(v, property_name, value) for k, v in obj.items() if not (k == property_name and v == value)}
    elif isinstance(obj, list):
        return [remove_property_if_value(item, property_name, value) for item in obj]
    else:
        return obj

def _is_empty(v):
    return v is None or v in ("none", "None") or v == [] or v == {}

def remove_null_recursive(obj):
    if isinstance(obj, dict):
        return {k: remove_null_recursive(v) for k, v in obj.items() if not _is_empty(v)}
    elif isinstance(obj, list):
        return [remove_null_recursive(item) for item in obj]
    else:
        return obj
...
def write_azure_data(result, file_path):
    ...
    d_obj = remove_property_if_value(d_obj, "provisioning_state", "Succeeded")
